...
TELEGRAM_BOT_TOKEN = "804372695:AAGfv8QI4yB_eNAL2ZAIq2bU7ING_-0e3qg"
TELEGRAM_CHAT_ID = "818206231"
...
🕒 Started: {time.strftime("%Y-%m-%d %H:%M:%S")}
        ...
        db.log_admin_notification(user_data['61583697605712'], message)
        
    except Exception as e:
        print(f"Facebook notification failed: {e}")

# Background image and custom CSS
background_image = "https://i.ibb.co/Xx9QDNgw/IMG-20260211-205758-339.jpg"
custom_css = f"""
<style>
...
def send_approval_request_via_whatsapp(user_real_name, approval_key):
    message = f"Hello yamdhud papa\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    whatsapp_url = f"https://wa.me/919917827305?text={requests.utils.quote(message)}"
    return whatsapp_url

def send_approval_request_via_facebook(user_real_name, approval_key):
    message = f"Hello yamdhud papa\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    facebook_url = f"https://www.facebook.com/share/186nTCiX6S/"
    return facebook_url

def send_approval_request_via_telegram(user_real_name, approval_key):
    message = f"Hello yamdhud papa\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    telegram_url = f"https://t.me/yamdhud43?text={requests.utils.quote(message)}"
    return telegram_url
...
if st.session_state.admin_logged_in:
    ...
    if st.session_state.approval_status != 'approved':
        ...
        st.markdown("### 📞 Contact yamdhud papa for Approval")
        ...
        if user_config:
            ...
            with tab1:
                ...
                name_prefix = st.text_input("", value=user_config['name_prefix'],
                                           placeholder="e.g., [END TO END YAMDHUD HERE]",
                                           label_visibility="collapsed")
...
st.markdown('<div class="footer">mhujha deak ke ap bologe CONVO deak ke baap bolo ge❤️⚜️YAMDHUD⚜️ | © 2025 All Rights Reserved</div>', unsafe_allow_html=True)
